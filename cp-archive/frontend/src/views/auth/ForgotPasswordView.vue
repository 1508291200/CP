    ...
    <div class="w-full max-w-sm">
      ...
      <div class="bg-[var(--color-bg-card)] rounded-[var(--radius-modal)] p-8 shadow-[var(--shadow-card)]">
          ...
          <form @submit.prevent="handleSendCode" class="flex flex-col gap-4">
            <div class="flex flex-col gap-1.5">
              ...
              <input
                ...
                :class="{ 'border-[var(--color-danger)]': emailError }"
              />
              <span v-if="emailError" class="text-xs text-[var(--color-danger)]">{{ emailError }}</span>
            </div>

            <div v-if="globalError" class="text-sm text-[var(--color-danger)] text-center bg-red-50 dark:bg-red-900/20 rounded-lg py-2 px-3">
              {{ globalError }}
            </div>
        ...
        <div v-else-if="step === 2">
          ...
          <p class="text-sm text-[var(--color-text-secondary)] mb-5">
            验证码已发送至 <span class="text-[var(--color-text-body)] font-medium">{{ maskedEmail }}</span>，请在10分钟内输入。
          </p>
          ...
          <form @submit.prevent="handleVerifyCode" class="flex flex-col gap-4">
            <div class="flex flex-col gap-1.5">
              ...
              <input
                ...
                class="w-full px-4 py-2.5 rounded-[var(--radius-input)] border border-[var(--color-border-input)] bg-[var(--color-bg-page)] text-[var(--color-text-body)] text-sm outline-none transition-colors focus:border-[var(--color-primary)] placeholder:text-[var(--color-text-disabled)] tracking-widest text-center text-lg"
                :class="{ 'border-[var(--color-danger)]': codeError }"
              />
            ...
            <div class="text-center text-sm">
              <span v-if="countdown > 0" class="text-[var(--color-text-disabled)]">
                {{ countdown }} 秒后可重新发送
              </span>
              <button
                v-else
                type="button"
                :disabled="resending"
                class="text-[var(--color-primary)] hover:underline disabled:opacity-60"
                @click="handleResend"
              >
          ...
          <form @submit.prevent="handleResetPassword" class="flex flex-col gap-4">
            <div class="flex flex-col gap-1.5">
              ...
              <input
                ...
                class="w-full px-4 py-2.5 rounded-[var(--radius-input)] border border-[var(--color-border-input)] bg-[var(--color-bg-page)] text-[var(--color-text-body)] text-sm outline-none transition-colors focus:border-[var(--color-primary)] placeholder:text-[var(--color-text-disabled)]"
                :class="{ 'border-[var(--color-danger)]': newPasswordError }"
              />
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <input
                ...
                autocomplete="new-password"
                class="w-full px-4 py-2.5 rounded-[var(--radius-input)] border border-[var(--color-border-input)] bg-[var(--color-bg-page)] text-[var(--color-text-body)] text-sm outline-none transition-colors focus:border-[var(--color-primary)] placeholder:text-[var(--color-text-disabled)]"
                :class="{ 'border-[var(--color-danger)]': confirmPasswordError }"
              />
        ...
        <div v-else-if="step === 4" class="text-center py-4">
          ...
          <p class="text-sm text-[var(--color-text-secondary)] mb-4">
            密码已重置，{{ redirectCountdown }} 秒后自动跳转到登录页。
          </p>
          <router-link
            to="/login"
...
async function handleVerifyCode() {
  ...
  try {
    ...
    if (err instanceof ApiClientError) {
      globalError.value = err.message === 'VALIDATION_ERROR' ? '验证码错误或已过期' : err.message
    } else {
      globalError.value = '验证失败，请稍后重试'
    }
  } finally {
    loading.value = false
  }
