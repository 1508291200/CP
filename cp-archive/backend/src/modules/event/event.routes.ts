...
export function buildEventRouter() {
  ...
  router.patch('/batch', permissionMiddleware('event:edit:own'),
  ...
  router.delete('/batch', permissionMiddleware('event:delete:own'),
  ...
  // ── 版本历史 ─────────────────────────────────────────────
  router.get('/:id/versions', permissionMiddleware('history:view:others'), async (c) =>
    c.json(success(await eventService.listEventVersions(c.req.param('cpId')!, c.req.param('id')!))))

  router.post('/:id/versions/:versionId/restore', permissionMiddleware('history:restore'), async (c) => {
